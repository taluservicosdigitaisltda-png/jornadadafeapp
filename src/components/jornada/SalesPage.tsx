...
export function Hero() {
  return (
    <section id="oferta-principal" className={`${container} grid items-center gap-12 py-14 lg:grid-cols-2 lg:gap-16 lg:py-20`} aria-labelledby="hero-titulo">
      <div>
        <BrandLogo width={250} />
        <p className="eyebrow mt-8 block">Cinco minutos de Palavra e oração guiada</p>
        ...
        <p className="mt-5 max-w-xl text-base leading-relaxed text-sand sm:text-lg">
          Com apenas cinco minutos, o Pausa de Fé ajuda você a parar, ouvir uma oração guiada e refletir sobre a Palavra — mesmo na correria e sem precisar saber como começar.
        </p>
        <p className="mt-4 max-w-xl font-display text-lg leading-relaxed text-gold-light">
          Uma pequena pausa não torna o dia perfeito. Mas pode mudar a forma como você atravessa o que vier.
        </p>
        <div className="mt-8 max-w-sm"><CheckoutButton location="hero">QUERO MINHA PAUSA DE FÉ</CheckoutButton></div>
...
const demos = [
  { theme: "Paz", title: "Pausa para acalmar o coração", duration: "5 min", excerpt: "Respire com calma. Você não precisa encontrar as palavras perfeitas. Por alguns minutos, apenas permaneça na presença de Deus." },
  { theme: "Direção", title: "Pausa para olhar o próximo passo", duration: "5 min", excerpt: "Nem toda resposta aparece de uma vez. Às vezes, a direção começa quando o coração desacelera." },
  { theme: "Esperança", title: "Pausa para recomeçar", duration: "5 min", excerpt: "Todo recomeço parece pequeno no início. Ainda assim, pode ser ali que a esperança volta a encontrar espaço." },
  { theme: "Gratidão", title: "Pausa para reconhecer o que sustenta você", duration: "5 min", excerpt: "Gratidão não apaga o que foi difícil. Ela abre espaço para perceber também aquilo que sustentou você." },
  { theme: "Força", title: "Pausa para os dias de cansaço", duration: "5 min", excerpt: "Você não precisa resolver tudo neste instante. Entregue a Deus o peso que não precisa carregar sozinho." },
];
    ...
    <section className={`${container} py-14 lg:py-20`} aria-labelledby="identificacao-titulo">
      <div className="grid items-center gap-9 lg:grid-cols-[1.05fr_0.95fr] lg:gap-14">
        ...
        <figure className="overflow-hidden rounded-[26px] border border-gold/30 bg-espresso shadow-2xl">
          ...
          <figcaption className="border-t border-gold/20 px-5 py-4 text-xs leading-relaxed text-sand">Cinco minutos podem ser o começo de uma pausa mais presente e verdadeira.</figcaption>
        </figure>
      </div>
    </section>
  );
...
const howItWorks = [
  ...
  { title: "Leve a pausa com você", text: "Guarde uma reflexão, escreva no diário ou apenas continue o dia com mais presença e intenção." },
];
...
const features = [
  ...
  { title: "Favoritos e histórico", text: "Retome as pausas que mais fizeram sentido para você." },
  ...
  return (
    <section className="relative border-y py-14 lg:py-20" style={{ background: "var(--gradient-brown)", borderColor: "oklch(0.76 0.106 79 / 25%)" }} aria-labelledby="caminho-titulo">
      <div className={container}>
        ...
        <h2 id="caminho-titulo" className="mt-4 max-w-3xl text-2xl text-ivory sm:text-3xl lg:text-4xl">Um caminho opcional de 28 dias para transformar pequenas pausas em constância</h2>
        <p className="mt-4 max-w-2xl text-base leading-relaxed text-sand">Você pode usar somente as pausas que precisar ou seguir uma sequência guiada. Não há metas rígidas nem dias bloqueados.</p>
        <figure className="relative mt-9 overflow-hidden rounded-[26px] border border-gold/30 shadow-2xl">
          ...
          <figcaption className="absolute inset-x-0 bottom-0 p-5 font-display text-base text-ivory sm:p-7 sm:text-lg">Uma pausa por vez. Um dia de cada vez.</figcaption>
        </figure>
        ...
        <p className="mt-8 flex items-start gap-3 text-base text-ivory"><ConfirmBadge />Se um dia não acontecer, sua Pausa de Fé continua disponível quando você quiser retomar.</p>
      </div>
    </section>
  );
}
...
const included = ["Aplicativo Pausa de Fé", "Biblioteca de orações e reflexões narradas", "Momentos guiados de aproximadamente 5 minutos", "Caminho opcional de 28 dias", "Diário, favoritos, histórico e transcrições", "PDFs de oração e reflexão", "Acesso vitalício ao conteúdo adquirido"];

export function Included() {
  return (
    <section className={`${container} py-14 lg:py-20`} aria-labelledby="incluido-titulo"><p className="eyebrow">Seu acesso</p><h2 id="incluido-titulo" className="mt-4 text-2xl text-ivory sm:text-3xl lg:text-4xl">Tudo que você recebe no Pausa de Fé</h2><ul className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">{included.map((item) => <li key={item} className="card-premium flex items-start gap-3 rounded-[18px] p-5 text-sm text-ivory"><ConfirmBadge /><span className="min-w-0">{item}</span></li>)}</ul></section>
  );
}
...
export function Offer() {
  ...
  return (
    <section id="oferta" ref={ref} className="relative overflow-hidden border-y py-14 lg:py-20" style={{ background: "var(--gradient-brown)", borderColor: "oklch(0.76 0.106 79 / 25%)" }} aria-labelledby="oferta-titulo">
      ...
      <div className={`${container} relative`}><div className="card-premium mx-auto max-w-xl rounded-[26px] p-7 text-center sm:p-10"><p className="eyebrow">Sua pausa pode começar hoje</p><h2 id="oferta-titulo" className="mt-4 text-2xl text-ivory sm:text-3xl">Acesso completo ao Pausa de Fé</h2><p className="mt-4 text-sm leading-relaxed text-sand">Cinco minutos para ouvir, refletir e reservar um pequeno espaço para Deus na sua rotina.</p><p className="mt-6 font-display text-5xl text-gold-light sm:text-6xl">R$ 29,90</p><p className="mt-3 text-sm text-sand">Pagamento único. Sem mensalidade para acessar o conteúdo adquirido.</p><div className="mt-7"><CheckoutButton location="oferta">QUERO MINHA PAUSA DE FÉ</CheckoutButton></div><p className="mt-4 text-xs text-sand">🔒 Compra processada em ambiente seguro pela Perfect Pay.</p><p className="mt-2 text-xs text-sand/80">Garantia de 7 dias conforme as condições apresentadas no checkout.</p></div></div>
    </section>
  );
}
